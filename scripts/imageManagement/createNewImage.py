...
def capture_image_pi(image_path, image_name, image_type, img_data):
    ...
    if image_type in valid_image_formats:
        command = f"fswebcam --{image_type} {img_data['quality']} -S 2 --save {image_path}/{image_name}.{image_type}"

    else:
        command = f"fswebcam --{image_type} {img_data['quality']} -S 2 --save {image_path}/{image_name}.jpeg"
